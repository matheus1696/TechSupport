...
<div class="px-2 pb-3">
    <form method="get" class="flex flex-col items-center gap-3 lg:flex-row">
        <div class="flex items-end gap-3">
            <label for="searchCod" class="text-sm font-semibold">Código CBO:</label>
            <input type="text" id="searchCod" name="searchCod" value="{{$search['searchCod'] ?? ''}}" class="flex-1 px-3 text-sm border border-gray-300 rounded-full shadow-sm bg-gray-100/50 focus:bg-white">
        </div>
        <div class="flex items-end flex-1 gap-3">
            <label for="searchName" class="text-sm font-semibold">Ocupação:</label>
