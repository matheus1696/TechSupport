        ...
        @slot('tbody')
            @foreach ($dbInventories as $dbInventory)
                <x-table.tr>
                    ...
                    <x-table.td>{{$dbInventory->Consumable->ConsumableType->title}}</x-table.td>
                                ...
                                <div class="text-left modal-dialog modal-lg" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="text-lg font-semibold modal-title" id="Modal">Saída de Produto: {{$dbInventory->Consumable->title}}</h5>
                                        ...
                                        <div class="m-4 modal-body">
                                            <x-form.form method="create" route="{{route('inventory_warehouses.exitStore')}}">
                                                ...
                                                <input hidden name="consumable_id" value="{{$dbInventory->consumable_id}}">

                                                <x-form.input col="12" label="Suprimento" name="disabled" value="{{$dbInventory->Consumable->title}}" disabled="disabled"/>
                                                ...
                                                <x-form.input col="3" label="Apresentação" name="disabled" value="{{$dbInventory->Consumable->ConsumableUnit->title}}" disabled="disabled"/>
