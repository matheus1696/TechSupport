...
<x-conteiner>
    ...
    <form method="POST" action="{{route('profile.update',['profile'=>$db->id])}}">
        ...
        <div class="form-row justify-content-center">
            ...
            <x-form.input col="12" label="Nome Completo" id="name" value="{{$db->name}}" required="required" />
            ...
            <x-form.input col="12" label="Departamento" id="disabled"
                value="{{$db->CompanyOrganizational->acronym ?? ''}} - {{$db->CompanyOrganizational->title ?? ''}}"
                disabled="disabled" />

            <x-form.input col="12" label="Cargo" id="disabled" value="{{$db->CompanyOccupations->occupation ?? ''}}"
                disabled="disabled" />
            <x-form.input col="12" label="Estabelecimento" id="disabled"
                value="{{$db->CompanyEstablishments->establishment ?? ''}}" disabled="disabled" />

            <x-form.input col="6" label="Contato 1" type="tel" id="contact_1" value="{{$db->contact_1}}"
                maxlength="15" minlength="13" />
            <x-form.input col="6" label="Contato 2" type="tel" id="contact_2" value="{{$db->contact_2}}"
                maxlength="15" minlength="13" />
